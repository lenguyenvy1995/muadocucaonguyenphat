...
<div class="product-thumbnail">
    ...
    >
        ...
        <div class="img-fluid-eq__wrap">
            <img
                class="lazyload product-thumbnail__img zoom-effect"
            ...
            @else
                @if ($product->productLabels->isNotEmpty())
                    @foreach ($product->productLabels as $label)
                        <span
                            class="ribbon"
                            @if ($label->color) style="background-color: {{ $label->color }} !important" @endif
                        >{{ $label->name }}</span>
                    @endforeach
                @else
        ...
        @if (is_plugin_active('marketplace') && $product->store->id)
            <div class="sold-by-meta">
                <a
                    ...
                    tabindex="0" title=" {{ $product->store->name }}"

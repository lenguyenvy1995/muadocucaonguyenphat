...
(function ($) {
  ...
  MartApp.toggleSidebarFilterProducts = function (
    status = "close",
    target = "product-categories-primary-sidebar"
  ) {
        ...
        $.ajax({
          ...
          success: (res) => {
            if (!res.error) {
              ...
              MartApp.productGallery(
                true,
                $modal.find(".product-modal-content .product-gallery")
              );
              MartApp.lightBox();
    ...
    MartApp.$body.on(
      ...
      function (e) {
        ...
        if ($this.hasClass("decrease") && current > min) {
          ...
          let $total2 = ($priceCurrent * 1 - $priceCurrent / current).toFixed(
            2
          );
          $price.html($total2);
        }
    ...
    $(document).on("click", ".btn-remove-coupon-code", (e) => {
      ...
      $.ajax({
        ...
        success: (res) => {
          if (!res.error) {
            $(".cart-page-content").load(
              window.location.href + " .cart-page-content > *",
              function () {
    ...
    if (window.siteConfig?.ajaxCart) {
      $.ajax({
        ...
        success: function (res) {
          if (!res.error) {
            $(".mini-cart-content .widget-shopping-cart-content").html(
              res.data.html
            );
            $(".btn-shopping-cart .header-item-counter").text(res.data.count);
            $(".cart--mini .cart-price-total .cart-amount span").text(
  ...
  MartApp.productsFilter = function () {
    ...
    function openCategoryFilter($li) {
      if (!$li) {
        ...
        if ($categories.length) {
          ...
          MartApp.$widgetProductCategories
            ...
            .removeClass("d-none");
          MartApp.$widgetProductCategories
            .find(".show-all-product-categories")
            .addClass("d-none");
        }
        MartApp.$widgetProductCategories
        ...
        $li
          .find("> .widget-layered-nav-list__item .nav-list__item-link")
          .removeClass("active");
      ...
      MartApp.$widgetProductCategories
        .find(".loading-skeleton")
        .removeClass("loading-skeleton");
    }

    openCategoryFilter();

    $(".catalog-toolbar__ordering input[name=sort-by]").on(
      "change",
      function (e) {
        MartApp.$formSearch
    ...
    $(document).on(
      "click",
      MartApp.widgetProductCategories + " li a",
      function (e) {
        e.preventDefault();
        ...
        if (categoryId) {
          let $item = $parent.find(
            ".widget-layered-nav-list .nav-list__item-link[data-id=" +
              categoryId +
              "]"
          );
          ...
          $parent
            .find(".widget-layered-nav-list .category-filter")
            .removeClass("opened d-none");
      ...
      $.ajax({
        ...
        success: function (res) {
          if (!res.error) {
            ...
            let title =
              res.additional?.category?.name ||
              MartApp.$formSearch.data("title");
        ...
        success: (res) => {
          if (!res.error) {
            if (url) {
              ...
              $panel
                .find(".panel__content")
                .append($content.find(".panel__content").contents());
            } else {
  ...
  MartApp.processUpdateCart = function ($this) {
    ...
    $.ajax({
      ...
      success: (res) => {
        ...
        $(".cart-page-content").load(
          window.siteConfig.cartUrl + " .cart-page-content > *",
          function () {
      ...
      $.ajax({
        ...
        success: (res) => {
          if (res.error) {
            ...
            MartApp.showSuccess(res.message);
            $(".btn-compare .header-item-counter").text(res.data.count);
            $(".compare-page-content").load(
              window.location.href + " .compare-page-content > *"
            );
          }
        },
        error: (res) => {
  ...
  MartApp.submitReviewProduct = function () {
    ...
    if (sessionStorage.reloadReviewsTab) {
      if ($('#product-detail-tabs a[href="#product-reviews"]').length) {
        new bootstrap.Tab(
          $('#product-detail-tabs a[href="#product-reviews"]')[0]
        ).show();
      }
  ...
  MartApp.vendorRegisterForm = function () {
    ...
    $("#shop-url-register")
      .on("keyup", function () {
        ...
        displayURL.html(
          displayURL.data("base-url") +
            "/<strong>" +
            $(this).val().substring(0, 100).toLowerCase() +
            "</strong>"
        );
      })
      .on("change", function () {
          ...
          success: (res) => {
            ...
            if (res.data?.slug) {
              displayURL.html(
                displayURL.data("base-url") +
                  "/<strong>" +
                  res.data.slug.substring(0, 100).toLowerCase() +
                  "</strong>"
              );
            }
          },
    ...
    const fetchData = (url, hasAnimation = false) => {
      $.ajax({
        ...
        success: function (res) {
          ...
          $(".product-reviews-container .product-reviews-header").html(
            res.message
          );
        ...
        if (res.error) {
          ...
          if (data.error_message) {
            ...
            $product
              .find(".number-items-available")
              .html(
                '<span class="text-danger">(' + data.error_message + ")</span>"
              )
              .show();
          } else if (data.success_message) {
            $product
              .find(".number-items-available")
              .html(res.data.stock_status_html)
              .show();
          } else {
            $product.find(".number-items-available").html("").hide();
          }
          ...
          if (unavailableAttributeIds && unavailableAttributeIds.length) {
            unavailableAttributeIds.map(function (id) {
              let $item = $product.find(
                '.attribute-swatch-item[data-id="' + id + '"]'
              );
              if ($item.length) {
                ...
                $item = $product.find(
                  '.product-filter-item option[data-id="' + id + '"]'
                );
                if ($item.length) {
